...
def fuzzy_match(official_names, category_names)
  matches = []
...
def associate_balance_sheet_names_v2(bdata, positions)
  puts "================================ ASSOCIATING category names with official bilanz position names ==="
...
def create_skr03_accounts_csv(all_codes)
  puts "========== STORING ACCOUNT CODES AND DESCRIPTIONS ===="

  parsed_codes = deduplicate_by_code(
    all_codes.map do |code_info|
      result = parse_code_string(code_info)
      if result[:code] == ""
        raise "ERROR! parse_code_string failed (no code detected) for #{code_info.inspect}"
      end
      result
    end.select { |pc| (PROG_FUNC_FLAGS & pc[:flags].split(" ")).size == 0 }
  )

  File.open("skr03-accounts.csv", "w") do |f|
    f.puts "# This file was automatically generated by contrib/parse_chart_of_accounts.rb"
    f.puts "# Columns: code; flags; range; description"
    parsed_codes.each do |pc|
      f.puts "#{pc[:code]};#{pc[:flags]};#{pc[:range]};#{pc[:description]}"
    end
  end
  puts "Updated skr03-accounts.csv"
end

def deduplicate_by_code(items)
  result = []
  ...
  items.group_by { |item| item[:code] }.each do |code, group|
    ...
    when 2
      ...
      without_wrong_flags = group.select { |i| (%w[KU V M] & i[:flags].split(" ")).size == 0 }
...
ocr_data.each do |row|
  ...
  if left_side_ignore_list.include?(pos_desc)
    ...
    if items
      ...
      all_codes += account_codes
    end
  end
end

puts "categories: #{positions.keys.size}. total items (account code [ranges]): #{all_codes.size}\n"
...
associate_balance_sheet_names_v2(bilanz_aktiva.merge(bilanz_passiva), positions)
