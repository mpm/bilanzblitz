...
The recommended workflow uses a two-stage process that allows manual review and correction of mappings:
...
The generated files will use hierarchical category IDs instead of SHA1 hashes.

### Alternative: Legacy Single-Step Approach

Run `parse_chart_of_accounts.rb` for the legacy single-step approach.

**Note**: This approach is less reliable because:
- It performs fuzzy matching without manual review
- You can't see which SKR03 categories were not matched
- Account codes may be silently missing from the output
- No opportunity to correct matching errors before generating final files

The two-stage approach (generate → review → build) is strongly recommended.

## Output Files

Both the legacy `parse_chart_of_accounts.rb` script and the recommended two-stage workflow (`generate_category_mapping.rb` → `build_category_json.rb`) generate the same output files with similar structure.

### Key Difference: Category IDs

- **Legacy approach**: Uses 7-character SHA1 hashes as category IDs (e.g., `"cid": "9f184e6"`)
- **New workflow**: Uses hierarchical path-based IDs (e.g., `"cid": "aktiva.anlagevermoegen.immaterielle.geschaeftswert"`)

The hierarchical IDs make it much easier to understand which category an account belongs to and to debug mapping issues.

### Generated Files

The new two-stage workflow generates four files:

### 0. category-mapping.yml (Intermediate File)
...
**Note**: The new workflow does not generate this file. Use `parse_chart_of_accounts.rb` if you need it.
...
**Key Attributes**:
- `cid`: Category identifier - either a 7-character SHA1 hash (legacy) or hierarchical path (new workflow). Null if no match found.
...
The category identifier (`cid`) serves as a stable identifier that links:
- Account codes in `skr03-accounts.csv` (legacy workflow only)
- Categories in `bilanz-with-categories.json`
- Categories in `guv-with-categories.json`

**Note**: The new workflow uses hierarchical IDs that make the category structure self-documenting (e.g., `aktiva.anlagevermoegen.immaterielle.geschaeftswert` clearly shows this is an asset under fixed assets under intangible assets). The legacy workflow uses 7-character hashes that are opaque but more compact.
